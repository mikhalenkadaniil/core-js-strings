...
function sumOfCodes(str) {
  if (str === '' || !str) return 0;
  return str.split('').reduce((accum, value) => accum + value.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((counter, char) => {
    if (VOWELS.includes(char)) return counter + 1;
    return counter;
  }, 0);
}
...
function encodeToRot13(str) {
  ...
  const NEW_ALPHABET = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((char) => {
      ...
      return NEW_ALPHABET[ALPHABET.indexOf(char)];
    })
    .join('');
}
